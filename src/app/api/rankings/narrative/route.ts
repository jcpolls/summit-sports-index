...
const DEMO_NARRATIVES: Record<string, string> = {
  ...
  alabama: "Alabama leads the AthletIQ with a 74.7 composite, driven by the nation's largest football budget, best-in-class EADA compliance, and dominant on-field results across multiple sports. The program's only material vulnerability is limited donor diversification outside the football enterprise.",
...
export async function POST(req: NextRequest) {
  ...
  if (demoMode || !process.env.ANTHROPIC_API_KEY) {
    const narrative = DEMO_NARRATIVES[institutionId] ?? `${schoolName} has a composite score of ${scores.composite} in the AthletIQ.`
    return NextResponse.json({ narrative })
  }

  const client = new Anthropic()

  const userPrompt = `${schoolName} ranks ${rank} of 5 in the AthletIQ with a composite score of ${scores.composite}. Dimension scores: financial=${scores.financial}, equity=${scores.equity}, academic=${scores.academic}, competitive=${scores.competitive}, reputation=${scores.reputation}, survey=${scores.survey}. Write a 2-sentence analyst summary.`
